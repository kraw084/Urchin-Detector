...
def forward(self, ims, size=640, augment=False, profile=False):
        ...
        with dt[0]:
            ...
            shape1 = (size, size) if isinstance(size, int) else (size[0], size[0])
            x = [letterbox(im, shape1, auto=False) for im in ims]  # pad
            padding_size = 32
            ratio_pad = [(r, (p[0] + padding_size//2, p[1] + padding_size//2)) for _, r, p in x]
            x = [im[0] for im in x]
            shape1 = (shape1[0] + padding_size, shape1[1] + padding_size)
            x = [letterbox(im, shape1, auto=False, scaleup=False)[0] for im in x]
